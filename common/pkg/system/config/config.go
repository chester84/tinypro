...
func Create(itemName, itemValue string, itemType types.SystemConfigItemType, weight int, description string, opid int64) (id int64, err error) {
	...
	if originID > 0 {
		...
		item.OfflineTime = libtools.GetUnixMillis()
		item.Utime = item.OfflineTime
		_, errUp := o.Update(&item)
		if errUp != nil {
			logs.Error("[SystemConfigCreate] db update exception, data: %#v, err: %v", item, errUp)
		}
	}
	...
	newItem := models.SystemConfig{
		...
		OpUid:       opid,
		OnlineTime:  libtools.GetUnixMillis(),
	}
	...
	hashName := hashKey()
	_, errRds := storageClient.Do("HSET", hashName, itemName, itemValue)
	if errRds != nil {
		logs.Error("[SystemConfigCreate] redis> HSET %s %s %s, err: %v", hashName, itemName, itemValue, errRds)
	}
...
func getValidItemValueFromCache(itemName string) (itemValue string, err error) {
	...
	if err != nil {
		...
		_, errRds := storageClient.Do("HSET", hashName, itemName, itemValue)
		if errRds != nil {
			logs.Error("[getValidItemValueFromCache] redis> HSET %s %s %s, err: %v", hashName, itemName, itemValue, errRds)
		}
	} else {
